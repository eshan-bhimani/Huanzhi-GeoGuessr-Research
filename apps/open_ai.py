"""
OpenAI API for vision+tools pipeline 
...
def exec_tool_until_image(
    ...
    *,
    ...
    """
    Execute tool with retries until image is obtained.
    ...
    Args:
        ...
        backoff_s: Base backoff time in seconds (multiplied by attempt number)
        last_image_path: Fallback image path if retries fail

    Returns:
        Tuple of (ok, image_path, updates, debug)

    Raises:
        RuntimeError: If no image obtained after retries and no fallback available
    """
...
def process_tool_call(
    ...
    log_path: Path,
    step: int,
) -> Tuple[bool, str, Optional[str], Optional[List[str]], List[str]]:
    """
    ...
    Returns:
        Tuple of (should_continue, new_image_path, final_answer, new_moves, context_lines)
    ...
    if final_from_tool is not None:
        ...
        return False, current_image_path, final_text, available_moves, []
    ...
    if invalid_reasoning(reasoning):
        ...
        return True, current_image_path, None, available_moves, []
    ...
    return True, new_image_path, None, new_moves, context_lines
...
def run_episode(
    ...
    *,
    ...
    recent_actions = []  # List of (tool_name, step) tuples
    ...
    for step in range(1, max_steps + 1):
        ...
        should_continue, new_image_path, final_answer, new_moves, context_lines = process_tool_call(
            env=env,
            messages=messages,
        ...
        recent_actions.append((tool_name, step))
        ...
        if len(recent_actions) >= 3:
            move_actions = [t for t, s in recent_actions if t.startswith("move_")]
            if len(move_actions) >= 3:
                # Check if mostly the same move
                last_move = move_actions[-1]
                same_move_count = sum(1 for m in move_actions[-3:] if m == last_move)
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="OpenAI vision+tools Street View navigator")
    ...
    # OpenAI config
    p.add_argument("--endpoint", type=str, default=os.getenv("AZURE_OPENAI_ENDPOINT"),
                   help="Azure OpenAI endpoint URL")
    p.add_argument("--api_key", type=str, default=os.getenv("AZURE_OPENAI_API_KEY"),
                   help="Azure OpenAI API key")
    p.add_argument("--model", type=str, default=os.getenv("AZURE_OPENAI_DEPLOYMENT", "gpt-4.1"),
